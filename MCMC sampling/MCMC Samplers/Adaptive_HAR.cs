...
namespace Preprocessing_MCMC_sampling
{
    class Adaptive_HAR
    {
        ...
        public Adaptive_HAR(NameValueCollection inputSettings, List<string> methodSettings, List<string> outputSettings, Dictionary<String, int[]> pathwayDataMat)
        {
            ...
            twFx = new StreamWriter(@"BNMCMC output\\LogFx_HAR_" + pathway + ".csv");
            ...
            for (; iter < nMaxIteration; iter++)
            {
                ...
                if ((fy - fx + Math.Log(Mu_X) - Math.Log(Mu_Y)) >= Math.Log(Ux))
                {
                    if (ch_AcceptRatio.Equals("MH"))
                    {
                        ...
                        twFx.WriteLine(fy.ToString() + "," + iter.ToString());
                        ListLogFx.Add(fy);
